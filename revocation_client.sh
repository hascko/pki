...
let i++
fichier=0$i.pem
cmp -s /$repertoire/$file1.pem /$repertoire/newcerts/$fichier

if [ $? -eq 0 ];then
	openssl ca -config /$repertoire/openssl.cnf -name $nom_certif -revoke /$repertoire/newcerts/$fichier
	openssl ca -gencrl -config /$repertoire/openssl.cnf -extensions CLIENT_RSA_SSL -name $nom_certif -crldays 1 -out /$repertoire/crl.pem
